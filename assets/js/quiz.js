...
function render(n) {
  quizHolder.innerHTML = `
        <span class="question-counter">Question ${data[n].id} sur ${
    data.length
  }</span>
    ...
    data[n].d
        ...
        <div class="btn-ctrl">
          <button type="button" onclick="nextQuestion(${n})" id="nextBtn">Suivant &nbsp;&rsaquo;</button>
        </div>
         `;
}
...
function checkAnswer(userAnswer, tag, n) {
  ...
  if (cA == 0) {
    if (userAnswer == data[n].answer) {
      ...
      tag.style.color = "#fff";
      tag.getElementsByTagName("i")[0].innerHTML += "<i>Réussi</i>";
      cA++;
      ...
      tag.style.color = "#fff";
      tag.getElementsByTagName("i")[0].innerHTML += "<i>Echoué</i>";

      corectTag = tag.parentElement.getElementsByTagName("li")[
        data[n].answer - 1
      ];
      corectTag.getElementsByTagName("i")[0].innerHTML += "<i>Réussi</i>";
